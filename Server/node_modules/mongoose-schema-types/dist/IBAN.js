...
function _testIBAN(iban, countryCode, structure) {
    return (structure.length === iban.length &&
        ...
        parseStructure(structure === null || structure === void 0 ? void 0 : structure.structure).test(iban.slice(4)) &&
...
class IBAN extends mongoose.SchemaType {
    constructor(key, options) {
        super(key, options, "IBAN");
    }
    cast(val) {
        if (!validate(val)) {
            throw mongoose.SchemaType.CastError("IBAN", `Value is not a valid IBAN: ${val}`);
        }
        return val;
    }
}
IBAN.instance = "IBAN";
mongoose.Schema.Types.IBAN = IBAN;
